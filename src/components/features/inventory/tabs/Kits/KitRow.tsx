...
export function KitRow({
    ...
    async function loadKitData() {
      try {
        ...
        if (occupation) {
          ...
          setAssemblyBinId(occupation.binId);
  ...
  const handleConfirmAssembly = async (qty: number, selectedBinId?: number) => {
    ...
    // Determine the final BIN ID to use
    // Priority: selectedBinId (from modal) > assemblyBinId (pre-assigned) > 0 (invalid)
    const finalBinId = selectedBinId || assemblyBinId;

    // Validate that we have a valid BIN ID (must be > 0)
    if (!finalBinId || finalBinId <= 0) {
      alert('Error: Please select a valid BIN location to build the kit.');
      return;
    }

    // Find the bin code for the selected bin
