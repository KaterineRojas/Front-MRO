...
import {
  ...
  SelectTrigger,
  SelectValue,
} from '@/components/features/ui/select';
import {
  Table,
  TableBody,
  TableCell,
  ...
  TableHeader,
  TableRow,
} from '@/components/features/ui/table';
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
  TooltipTrigger,
} from '@/components/features/ui/tooltip';
import {
  ArrowUpDown,
  Download,
  X,
        ...
        {!loading && filteredTransactions.length > 0 && (
          <>
            ...
            <div className="md:hidden space-y-3">
              {paginatedTransactions.map((transaction) => {
                const { line1, line2 } = formatTransactionDate(transaction.createdAt);
                const { displayText, isPositive } = formatQuantityChange(transaction.quantityChange);
                  ...
                  <Card key={transaction.id}>
                    <CardContent className="pt-4 space-y-3">
                      ...
                      {showItemColumn && (
                        <button
                          onClick={() => onItemClick?.(transaction.itemSku)}
                          className="w-full text-left p-2 rounded hover:bg-muted focus:outline-none focus:ring-2 focus:ring-primary"
                        >
                          <div className="font-medium">{transaction.itemName}</div>
                          <div className="text-xs text-muted-foreground font-mono">{transaction.itemSku}</div>
                        </button>
